...
OpenFlow 是最出名的但是不是唯一的 SDN 南向接口。其他推荐的 API 包括 ForCES [30], OVSDB [153], POF [31], [120], OpFlex [154], Openstate [155], ROFL [156], HAL [157], [158], PAD [159]. ForCES 提供了与传统网络管理间更宽松的连接，不需要改变当前网络架构，包括，不需要一个逻辑中心化的控制器：控制层与数据层是分离的，但是也可以放在同一个网络设备中。
